...
$p='/吃苹果/';			//要匹配/搜索的内容，常用模式匹配  $p='/定义一个模式/'  
$str='我喜欢吃苹果';	//在$str里搜索
...
//^符号在方括号里面表示字符类取反， 结尾\s表示匹配任意的空白符，包括空格，制表符，换行符
//^在最左边表示开头，$在最右边表示结尾！
...
//[^>]表示不为>的任意一个字符
//(.*?)解析：点 是任意字符；* 是取 0 至 无限长度；问号 是非贪婪模式。合在一起就是 取尽量少的任意字符；
//i：ignorCase忽略大小写；m：mutiple允许多行匹配；g：globle进行全局匹配，指匹配到目标串的结尾；
